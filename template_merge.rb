class TemplateMerge
  ...
  def render_to_file(output_path)
    raise NotImplementedError.new("Please call this only on a specific implementation of TemplateMerge, such as TemplateMergeSingle.")
  end
  ...
  def log_errors
    MotiaTools.logger.error("Problems with #{self.class.name}. Details below.\n- #{errors.join("\n- ")}")
    :error
  end
